...
if __name__ == '__main__':
    ...
    # Learning rate schedule with empirical warm-up
    def get_lr(iter_num, warmup_iters=500):
        # Linear warmup
        if iter_num < warmup_iters:
            return config.learning_rate * iter_num / warmup_iters
        # After warmup, use empirical scaling based on loss improvement
        return config.min_lr
    
    # Training loop
    iter_num = 0
    best_val_loss = float('inf')
    X, Y = trainer.get_batch('train')
